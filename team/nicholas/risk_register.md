...
| T6 | **Stale `feeds.json` between regeneration and demo** — `feeds.json` is generated *once* by `scripts/generate_feeds.py` (fusion runs at generation time, not on dashboard load). If the laptop sleeps or the run is hours before the demo, the marker timestamps inside `feeds.json` look stale relative to wall-clock to a security-minded judge eyeballing the JSON. *(Note: `feeds.json` itself is the contract — fusion is not re-run by the dashboard, so freshness re-evaluation does not occur. The earlier proposal to bump `window_s` to 600s as a mitigation is **rejected** because it would weaken the freshness security claim in production. The real mitigation is regenerating right before the demo.)* | P2 | L | Run `python scripts/generate_feeds.py` immediately before demo (and after every laptop sleep / standby). Add to the pre-demo checklist. | Re-run the script — ~2 seconds. |
...
| P4 | **"Is this just YOLO with extra steps?"** Judges who skim the visual classifier conclude the demo is a thin wrapper around a pretrained drone detector. | P1 | M | **ML is shipped + gated but explicitly cut from the demo path** (see `docs/judge_faq.md` Q6). Lead the pitch with HMAC-marker + VBI as the novel element. Cite the safety invariant: `_resolve_label()` enforces that `known_friendly_*` labels can never come from YOLO — only from hand-labels confirmed against the HMAC marker. The model contributes nothing to `FRIENDLY_VERIFIED` by construction. Birger Q2 explicitly said no classifier needed. | "We built ML, and we *gated* it. Pretrained YOLO can never produce a friendly label in our system — that's a code-enforced safety invariant. The marker is authoritative; ML is supporting evidence only, and we cut it from the live demo because Birger said we didn't need it. Responsible AI by construction." |
...
| P7 | **"Does this actually fit Problem Statement 2?"** A judge scoring against PS2 (edge deployments) asks how the project maps to the challenge framing. | P0 | M | Lead pitch with: *edge-on-edge, software-only, no added drone hardware, backpack-portable receiver, no cloud / no central server, ~16ms latency, runs on what's already in the field.* `docs/steganographic_iff.md` §"Why this fits Natsec Problem Statement 2" is the cite-list. | Read the table directly: drone-side compute = STM32, receiver-side = Pi/laptop, both deliberately low-to-moderate compute. Not a swarm-AI play, not a mission-execution play — IFF specifically. |
